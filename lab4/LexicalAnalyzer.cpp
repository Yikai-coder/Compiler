    ...
    while (it!= program.end())
    {
        ...
        if(!inSingleQuote && !inDoubleQuote)
        {
            switch(chr)
            {
                ...
                {
                    // 前一个字符是'/'，再接收到一个'/'，为单行注释
                    if(it != program.begin() && *(it-1)=='/')
                    {
                        string::iterator tmp_it = it + 1;
                        while ((tmp_it+1)!=program.end() &&  *tmp_it != '\n')
                            tmp_it++;
                ...
                case '*':
                {
                    ...
                    if(it != program.begin()  && *(it - 1) =='/')
                    {
                        ...
                        {
                            if (*tmp_it == '*')
                                if ((tmp_it + 1)!=program.end() && *(tmp_it + 1) == '/')
                                    break;
                            tmp_it++;
                        }
    ...
    while(idx < len)
    {
        ...
        else if(isalpha(chr) || chr=='_')
        {
            ...
            if(cit==this->token_table.end())
            {
                ...
                {
                    symbol_table_element element(token);
                    this->symbol_table.push_back(element);
...
int LexicalAnalyzer::CheckSymbolTable(string & token)
{
    ...
    {
        if(this->symbol_table[i].name==token)
            return i;
    }
    return -1;
}
 ...
 * @retval None
 */
void LexicalAnalyzer::OutputSymbolAndToken(string & outputFile)
{
    ...
    for(int i = 0; i < symbol_table.size(); i++)
        ofs<<'('<<i<<", "<<symbol_table[i].name<<')'<<endl;
    ofs.close();
}
